...
def main():
    ...
    # Get the arguments
    args = docopt("""Parse the text file and create a triplets file, each line is formatted as follows: X\t\Y\tpath

    Usage:
        parse_wikipedia.py <wiki_file> <out_file>

        <wiki_file> = the text file
        ...
        with codecs.open(out_file, 'w', 'utf-8') as f_out:
            ...
            for paragraph in f_in:
                ...
                for sent in parsed_par.sents:
                    dependency_paths = parse_sentence(sent,f_out)
...
def parse_sentence(sent,f_out):
    ...
    #           if len(token.orth_) > 2  and token.pos_ in ['NOUN', 'VERB', 'ADJ']]

    # GB -- added filter for 1 word tokesn that requires tag_ == "NN" etc 50% reduction in paths (for movie vocab)
        # restricted noun chunks to multiword tokens (others already covered by single word nouns case 1)
        # GB -- made dictionary look-up case insensitive!!!
    # Get all term indicesi
    #print >> f_out, "sentence {} {}".format(sent.start,sent.end)
    #for i, token in enumerate(sent) :
    #    try : 
    #        print >> f_out,  "{}\t{}".format(token,token.i)
    #    except UnicodeDecodeError :
    #        True
    #for chunk in sent.doc.noun_chunks :
    #    try : 
    #        if sent.start <= chunk.start < sent.end :
    #            print >> f_out,  "{}\t{}\t{}".format(chunk.start,chunk.end,chunk.orth_)
    #    except UnicodeDecodeError :
    #        True
    #    except UnicodeEncodeError :
    #        True
    # indices = [("_".join([token.orth_.lower(),str(token.i),str(token.i+1)]),sent[ind:ind+1], token.i, token.i+1) for ind, token in enumerate(sent)
    #            if len(token.orth_) > 2 and 
    # ( token.tag_ == "NN" or token.tag_ == "NNP" or token.tag_ == "NNS" or token.tag == "NNPS" or token.tag_ == "CD")]
    # indices.extend([("_".join([np.orth_.lower(),str(np.start),str(np.end)]), np, np.start, np.end) for np in sent.doc.noun_chunks 
    #   if np.end - np.start > 1 and sent.start <= np.start < sent.end 
    #  ])
    indices = [token.orth_.lower(),sent[ind:ind+1], token.i, token.i+1) for ind, token in enumerate(sent)
               if len(token.orth_) > 2 and 
    ( token.tag_ == "NN" or token.tag_ == "NNP" or token.tag_ == "NNS" or token.tag == "NNPS" or token.tag_ == "CD")]
    indices.extend([np.orth_.lower(), np, np.start, np.end) for np in sent.doc.noun_chunks 
      if np.end - np.start > 1 and sent.start <= np.start < sent.end 
     ])
